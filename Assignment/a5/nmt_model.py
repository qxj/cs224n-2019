...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)  # (src_len, b)

        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)  # (src_len, b, m_word_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)  # (src_len, b, m_word_len)
